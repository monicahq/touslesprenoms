    ...
    <div class="mx-auto max-w-2xl px-2 sm:px-6 lg:px-8 py-2 mb-10">
      ...
      <form method="POST" action="{{ route('search.post') }}" class="mb-4" x-data="{ name: '{{ $term }}' }">
        ...
        <div class="flex items-center px-2 py-2 bg-gray-100 rounded-lg border-gray-300 border">
          ...
          <button type="submit" x-bind:disabled="name.length < 3" hx-post="{{ route('search.post') }}" hx-target="#content" class="disabled:bg-red-100 disabled:text-gray-600 border-t border-b border-r border-gray-300 bg-gray-100 dark:bg-gray-700 rounded-r-lg px-4 py-2 text-gray-900 dark:text-gray-100 font-bold">Rechercher</button>
        </div>
      </form>
