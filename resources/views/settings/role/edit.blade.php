<x-app-layout>
  <x-slot name="breadcrumb">
    <ul class="text-sm">
      ...
      <li class="inline">{{ __('Edit a role') }}</li>
    </ul>
  </x-slot>
  ...
  <div class="py-4 sm:py-12">
    <div class="mx-auto max-w-xl px-2 sm:px-6 lg:px-8">
      <div class="overflow-hidden bg-white shadow-sm dark:bg-gray-800 rounded sm:rounded-lg">
        <form method="POST" action="{{ route('settings.role.update', $data['id']) }}">
          @csrf
          @method('PUT')

          <div class="relative border-b dark:border-gray-600 px-6 py-4">
            <h1 class="text-center text-lg font-bold">{{ __('Edit role') }}</h1>
          </div>

          <div class="relative px-6 py-4">
            <x-input-label for="name"
                          :value="__('What is the name of the role?')" />
